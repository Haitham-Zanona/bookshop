...
// $name = $this->onedrive_folder ? $this->onedrive_folder.'/'.$file : $file;
	 ...
	 *
	 * @return String|WP_Error - the pointer of the given folder, otherwise WP_Error if an exception occurs or the folder doesn't exist
	 */
				...
				if ($pointer == $new_pointer) {
					if ($create_if_not_exists) {
						...
						if ($newdir) {
							...
							return new WP_Error('container_access_error');
						}
					} else {
	...
	public function do_delete($files) {
		...
		if (is_object($storage) && !is_wp_error($storage)) {
			...
			if (is_wp_error($pointer)) return $pointer->get_error_code();
